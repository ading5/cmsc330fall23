...
### `read_codons(file)`
- **Description**: Given a path to a particular file, read all the contents of the file and store them in a data structure of your choice. Your data structure should be cleaned at the beginning of each read.
    - The amino acid name left of the colon **always starts** with an uppercase letter and can be followed by any number of lowercase or uppercase letters. The colon touches the right side of the amino acid name and is followed by a space. The number of sequences for a single amino acid can range from 1 to infinity, and are always separated by a comma and a space.
    - The valid sequence itself has at least 1 character of "A", "G", "U" or "C", and must all be uppercase. Any other characters are invalid (For example the Asparagine line is invalid, as it contains a stray 9 character).
    - Each codon will be on its own line in the file. If **any sequence is invalid**, the entire line should be ignored.
    - For this project, you can assume the sequences for each codon are **unique to that codon** and there won't be overlap.
...
- As an added challenge, entries in the file can contain the regex pattern `\{\d+\}`, which will have an effect on the entry. Consider the example `codons7.txt` file below:
```text
  Serine: A{4}GU
  ...
  Leucine: G{6}U{58}A
```
The number inside the curly brackets replaces the previous letter that number of times. For example, **the sequence corresponding to the Serine amino acid is AAAAGU**, because the A has {4} after it.  
...
### `encode(sequence)`
- **Description**: Given a string of amino acid names separated by spaces, create the RNA sequence, reading from left to right.
- If a codon does not exist, skip over it.
- If a particular amino acid has several sequences, **take the longest one**.
- If there are multiple longest sequences, return either (you only need to return one potential correct answer).
- **Type**: `str -> str`
...
### `decode(sequence)`
- **Description**: Given an RNA sequence from left to right, create a string of the amino acids separated by spaces.
...
because GGG is valid, but CCU is not valid so it moves on to CUA which is valid.
  ...
  - If there is noise, **skip over it**.
  - Precedence is to **take the longest sequence first**.
...
### `read_evals(file)`
- **Description**: Given a path to a particular file, read all the contents of the file and store them in a data structure of your choice, which will be used when the `operate()` function is called. 
- Your data structure should be cleaned at the beginning of each read.
- A valid order name consists of 1 or more alphanumeric characters (either upper or lowercase).
- The valid read order will be either an "L" or an "R". A valid operation order will be either "PO", "PR", or "I".
- The colon touches the right side of the order name and is followed by a space. A comma and a space (in that order) separate the read order and the operation order.
- Each order will be on its own line in the file. If a line is invalid, **ignore it**.
- Here is an example of an `eval.txt` file:
```text
  Order1: L, PO
  Order2: R, PR
  Order3: L, I
...
- **Description**: Given a RNA sequence (with possible noise) and a name of an operation structure, return a RNA sequence having performed all operations on said sequence. See the [Building the Protein](#building-the-protein) section for more information.
- If `eval_name` is not valid, return `None`. 
...
### START and STOP
- START and STOP dictate when the operator should build an amino acid chain. When a START is reached, 
